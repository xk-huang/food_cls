...
from datasets.food import Food_LT
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    elif config.dataset == 'imagenet' or config.dataset == 'ina2018' or config.dataset == 'food':
    ...
    if config.dataset == 'cifar10':
        ...
        dataset = Food_LT(config.distributed, root=config.data_path,
                          batch_size=config.batch_size, num_works=config.workers)
    ...
    validate(val_loader, model, classifier, lws_model, criterion, config, logger, block, dataset.test_names)


def validate(val_loader, model, classifier, lws_model, criterion, config, logger, block=None, test_names=None):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            all_outputs.append(output.argmax(-1).cpu().numpy())
